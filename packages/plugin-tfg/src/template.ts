...
4. "data": **optional** transaction data field (hex‑encoded calldata). Accept either:
   • A string already starting with "0x" (assumed to be valid hex).  
   • Plain‑text ASCII/UTF‑8, in which case convert it to hex (prefix with "0x").
...
1. Identify the relevant parts of the user's message:
   ...
   - Quote the segment that mentions 'data', if present.
...
2. Validate the extracted information:
   ...
   - For "data", if provided and doesn't start with "0x", convert ASCII → hex and prefix with "0x".
...
\`\`\`json
{
  ...
  "toAddress": string,
  "amount": string,
  "data": string // optional, may be an empty string if not supplied
}
\`\`\`
